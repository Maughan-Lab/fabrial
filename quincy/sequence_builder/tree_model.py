...
from .tree_item import TreeItem, CategoryItem

JSON = "application/json"


class TreeModel[ItemType: TreeItem](QAbstractItemModel):
    """
    ...
    def __init__(self, name: str, items: Sequence[TreeItem]):
        QAbstractItemModel.__init__(self)
        self.name = name
        self.root_item = CategoryItem.create_root(items)

        self.base_flag = Qt.ItemFlag.ItemIsEnabled

    def get_item(self, index: QModelIndex) -> ItemType:
        """
        ...
        if index.isValid():
            ...
            item: ItemType = index.internalPointer()
            if item is not None:
                return item
    ...
    # ----------------------------------------------------------------------------------------------
    # overridden methods
    def parent(self, index: QModelIndex) -> QModelIndex:  # type: ignore # overridden
        ...
        parent_row = parent_parent_item.index(parent_item)
...
class OptionsModel(TreeModel):
    ...
    def init_from_directory(self, directory_path: PathLike | str) -> Self:
        ...
        return self.init_from_dict(sequence_builder.item_dict_from_directory(directory_path))
