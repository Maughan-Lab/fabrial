...
class MainWindow(QMainWindow):
    def __init__(self, parent: QWidget = None):
        ...
        self.model = TreeModel("Jimmy", self)
        self.model.root_item.insert_children(0, 5)
        ...
        self.view2 = SequenceTreeView()
        self.view2.setAlternatingRowColors(True)
        self.view2.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectItems)
        self.view2.setHorizontalScrollMode(QAbstractItemView.ScrollMode.ScrollPerPixel)
        self.view2.setAnimated(False)
        self.view2.setAllColumnsShowFocus(True)
        layout.addWidget(self.view2)

        self.model2 = TreeModel("Pam", self)
        self.model2.root_item.insert_children(0, 5)

        self.view2.setModel(self.model2)
        self.view2.expandAll()

        for column in range(self.model2.columnCount()):
            self.view2.resizeColumnToContents(column)
    ...
    def insert_child(self) -> None:
        ...
        for column in range(model.columnCount(index)):
            ...
            if not model.headerData(column, Qt.Orientation.Horizontal):
                model.setHeaderData(
                    column, Qt.Orientation.Horizontal, "[No header]", Qt.ItemDataRole.EditRole
                )
    ...
    def insert_column(self) -> None:
        ...
        if changed:
            model.setHeaderData(
                column + 1, Qt.Orientation.Horizontal, "[No header]", Qt.ItemDataRole.EditRole
            )
